...
export default class TodoMemberList extends Component {
  render() {
    ...
    this.store.teamTodoList.forEach((todoList) => {
      const todoListContainer = this.createDOM(todoList);
      this.$target.appendChild(todoListContainer);
      TodoListView(todoListContainer, todoList, this.store.teamId);
    });
